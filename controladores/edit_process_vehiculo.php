...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $vehiculo = $collection->findOne(['_id' => new MongoDB\BSON\ObjectId($id)]);
    ...
    $updateData = [];
    if ($vehiculo['placa'] !== $placa) $updateData['placa'] = $placa;
    if ($vehiculo['marca'] !== $marca) $updateData['marca'] = $marca;
    if ($vehiculo['modelo'] !== $modelo) $updateData['modelo'] = $modelo;
    if ($vehiculo['anio'] !== intval($anio)) $updateData['anio'] = intval($anio);
    if ($vehiculo['tipo'] !== $tipo) $updateData['tipo'] = $tipo;
    if ($vehiculo['clase'] !== $clase) $updateData['clase'] = $clase;
    if ($vehiculo['numero_chasis'] !== $numero_chasis) $updateData['numero_chasis'] = $numero_chasis;
    if ($vehiculo['numero_motor'] !== $numero_motor) $updateData['numero_motor'] = $numero_motor;
    if ($vehiculo['propietario_id'] != $propietarioObjectId) $updateData['propietario_id'] = $propietarioObjectId;
